...
WHERE membercost > 0 AND membercost < monthlymaintenance * (1.0 / 50.0);
...
SELECT memid, surname, firstname, joindate FROM cd.members WHERE DATE(joindate) >= '2012-09-01';
...
SELECT joindate FROM cd.members WHERE joindate = (SELECT MAX(joindate) FROM cd.members);
...
WHERE name ILIKE '%tennis%' AND DATE(starttime) = '2012-09-21'
